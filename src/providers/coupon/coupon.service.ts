  ...
  apply = function (coupon, lineItems) {
    ...
    if (coupon.discount_type == 'fixed_cart') {
      ...
      skip=false;
        ...
        coupon.excluded_product_categories.forEach(element => {
          if(value.categories_id==+element){
            skip=true;
          }
        });
        if( coupon.exclude_product_ids.length)
        coupon.exclude_product_ids.forEach(element => {
          if(value.products_id==+element){
            skip=true;
          }
        });
        if(!skip){
